...
public:
	...
	static constexpr double TIME_RES     = 0.000015650040064103f;
	static constexpr double TIME_RES_INV = 63897.6f;

	// Speed of radio waves [m/s] * timestamp resolution [~15.65ps] of DW1000
	static constexpr double DISTANCE_OF_RADIO     = 0.0046917639786159f;
	static constexpr double DISTANCE_OF_RADIO_INV = 213.139451293f;
	...
	// TODO use non float
	static constexpr double SECONDS      = 1e6;
	static constexpr double MILLISECONDS = 1e3;
	static constexpr double MICROSECONDS = 1;
	static constexpr double NANOSECONDS  = 1e-3;

	// constructor
	DW1000Time();
	DW1000Time(uint64_t time);
	DW1000Time(byte data[]);
	DW1000Time(const DW1000Time& copy);
	DW1000Time(double timeUs);
	DW1000Time(uint64_t value, double factorUs);
	~DW1000Time();
	...
	// real time in us
	void setTime(double timeUs);
	void setTime(uint64_t value, double factorUs);
