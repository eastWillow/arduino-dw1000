...
void DW1000Mac::generateShortMACFrame(byte frame[], byte sourceShortAddress[], byte destinationShortAddress[], byte network[])
{
	...
	*(frame + 2) = _seqNumber;
	//Destination PAN ID
	memcpy(frame + 3, network, 2);

	//destination address (2 bytes)
	memcpy(frame + 5, destinationShortAddress, 2);

	//source PAN ID
	//memcpy(frame+7, network, 2);
...
//Short Mac Frame
void DW1000Mac::generateShortMACFrame(byte fc, byte frame[], byte sourceShortAddress[], byte destinationShortAddress[], byte network[])
{
	//Frame controle
	*frame = fc;
	*(frame + 1) = FC_2_SHORT;
...
void DW1000Mac::decodeShortMACFrame(byte frame[], byte destAddr[], byte srcAddr[])
{
	decodeShortMACFrame(frame, srcAddr);
	//we grab the destination address for the mac frame
	byte destinationAddress[2];
	memcpy(destAddr, frame + 5, 2);
}

void DW1000Mac::decodeShortMACFrame(byte* fc, byte frame[], byte sourceShortAddress[], byte destinationShortAddress[], byte network[])
{

}

void DW1000Mac::decodeShortMACFrame(byte frame[], byte sourceShortAddress[], byte destinationShortAddress[], byte network[])
{
	memcpy(network, frame+3, 2);
	memcpy(destinationShortAddress, frame + 5, 2);
	memcpy(sourceShortAddress,frame + 7, 2);
}
...
void DW1000Mac::decodeLongMACFrame(byte frame[], byte destAddr[], byte srcAddr[])
{
	decodeLongMACFrame(frame, srcAddr);
	//we grab the destination address for the mac frame
	byte destinationAddress[8];
	memcpy(destAddr, frame + 5, 8);
}
