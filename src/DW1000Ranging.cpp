...
void (* DW1000RangingClass::_handleUnknownMessage)(byte(*)[90],uint16_t) = 0;
void (* DW1000RangingClass::_handleSafeTransmit)(DW1000Device*) = 0;
...
void DW1000RangingClass::loop() {
	...
	if(_receivedAck) {
		...
		if(messageType == BLINK && _type == ANCHOR) {
			...
			// Check if we have a valid device, given our incoming address
