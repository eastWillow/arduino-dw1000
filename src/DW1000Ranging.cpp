...
int32_t DW1000RangingClass::last_time = 0;
...
void DW1000RangingClass::loop()
{
	...
	checkForReset();
	uint32_t now_time = millis(); // TODO other name - too close to "timer"
	if (now_time - last_time > _timerDelay)
	{
		last_time = now_time;
		timerTick();
	}
...
void DW1000RangingClass::loopwoReport()
{
	//we check if needed to reset !
	checkForReset();
	uint32_t now_time = millis(); // TODO other name - too close to "timer"
...
void DW1000RangingClass::Tag_loop()
{
	...
	if (_receivedAck)
	{
		...
		if (messageType == RANGING_INIT && _type == TAG)
		{
			Serial.println("3_GET_RANGINGINIT");
			byte address[2];
			_globalMac.decodeLongMACFrame(data, address);
			//we crate a new device with the anchor
			DW1000Device myAnchor(address, true);
...
void DW1000RangingClass::Anchor_loop()
{
	//we check if needed to reset !
	checkForReset();
	uint32_t now_time = millis(); // TODO other name - too close to "timer"
	...
	if (_receivedAck)
	{
		...
		if (messageType == BLINK && _type == ANCHOR)
		{
			...
			if (addNetworkDevices(&myTag))
			{
				...
				//we reply by the transmit ranging init message
				transmitRangingInit(&myTag);
				Serial.println("2_SEND_RANGINIT");
				noteActivity();
			}
			_expectedMsgId = POLL;
		}
...
void DW1000RangingClass::transmitPoll(DW1000Device *myDistantDevice)
{
	...
	else
	{
		...
		myDistantDevice->setReplyTime((2 * 0 + 1) * DEFAULT_REPLY_DELAY_TIME);
		memcpy(data + SHORT_MAC_LEN + 2 + 4 * 0, myDistantDevice->getByteShortAddress(), 2);

		uint16_t replyTime = myDistantDevice->getReplyTime();
		memcpy(data + SHORT_MAC_LEN + 2 + 2 + 4 * 0, &replyTime, 2);
